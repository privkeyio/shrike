...
- **Fee rates and explorer links follow this chain.** mempool.space follows the chain that kept SHA256d, so past activation it holds neither this chain's blocks nor its mempool: a block lookup returns nothing, and a fee estimate prices against a mempool this wallet never broadcasts into. The default for both is mempool.guide, which runs the same API on this chain. The others stay selectable in the settings for anyone who wants to look at the other chain.
- **Only Knots can corroborate the schedule.** The activation height is read from `getdeploymentinfo`, which only Knots reports. Other nodes and Electrum servers still work and the wallet still opts in against them, because declining whenever a server cannot answer would forgo the protection on every such connection. It says on the send screen that the height went unchecked, so a build shipping a stale one would not be caught there.
